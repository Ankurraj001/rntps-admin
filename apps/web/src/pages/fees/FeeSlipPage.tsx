...
export function FeeSlipPage() {
  ...
  /*
    The slip is reached from two places — the invoice page and a student's Fees tab — so a
    fixed destination is wrong from one of them. Going back a step returns to whichever it
    was.

    'default' is the key React Router gives the entry the app was loaded on, so there is no
    in-app step to return to: a fresh tab, a reloaded deep link, or the printed slip's URL
    typed in directly. Stepping back there would leave the app entirely, so those fall
